...
(async function init() {
  let user;
  try {
    user = await fetch('/api/me').then(r => {
...
async function loadProjects() {
  ...
  try {
    projects = await fetch('/api/admin/projects').then(r => {
...
async function saveProject() {
  ...
  try {
    if (editingId) {
      await fetchOrThrow(`/api/admin/projects/${encodeURIComponent(editingId)}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, aliases, active }),
      });
    } else {
      await fetchOrThrow('/api/admin/projects', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
async function toggleActive(id) {
  ...
  if (!p) return;
  try {
    await fetchOrThrow(`/api/admin/projects/${encodeURIComponent(id)}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
async function deleteProject(id) {
  ...
  try {
    await fetchOrThrow(`/api/admin/projects/${encodeURIComponent(id)}`, { method: 'DELETE' });
    await loadProjects();
  } catch (err) {
    alert(err.message);
  }
